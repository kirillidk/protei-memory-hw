...
std::vector<
    std::pair<char const*, matrix (*)(matrix const&, matrix const&)>> const
    multiply_functions = {
        // {"wrong_multiply", wrong_multiply},
        // {"basic_multiply", basic_multiply},
        {"optimised_multiply", optimised_multiply},
};
...
int main() {
    ...
    size_t const big_r = 4096;
    size_t const big_c = 4096;
    ...
    for (auto const& [owner_name, multiply_func] : multiply_functions) {
        ...
        if (not basic_test(multiply_func)) {
            std::cout << "Function by " << owner_name
                      << " failed in basic test and rejected!" << std::endl;
            continue;
        } else {
            std::cout << "Function by " << owner_name
                      << " passed basic test! Continue..." << std::endl;
        }
        ...
        //         if ((_i == _j  and result[_i][_j] != 1) or (_i != _j and
        //         result[_i][_j] != 0)) {
        //             result_times[i] = {};
        //         }
        //     }
    ...
    std::cout << '\n';
    result_printer << '|' << std::setw(24) << ' ' << "RESULTS" << std::setw(24)
                   << ' ' << '|' << std::endl;
    for (i = 0; i < multiply_functions.size(); ++i) {
        ...
        if (result_time.tv_nsec == 0 and
            result_time.tv_nsec == result_time.tv_sec) {
            result_printer << std::setw(23) << "failed |" << std::endl;
        } else {
            result_printer << std::setw(3) << result_time.tv_sec << "sec "
                           << std::setw(10) << result_time.tv_nsec << "nsec |"
                           << std::endl;
        }
    }
